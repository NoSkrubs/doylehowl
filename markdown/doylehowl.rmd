...
With **`r comma(nrow(filter(data, page == 'Reed Releases')))`** total posts Reed Releases was the shortest lived amongst the three pages. It lasted exactly three months, from its first post to its signing off notifcation in February 2013. Due to the winter break in the middle though, Reed Releases popularity was mostly confined to a few weeks over finals.
...
```{r}
reed_releases_first_two_weeks <- data %>% 
  filter(
     day <= '2012-12-20'
    ,page == 'Reed Releases'
  ) %>%
...
reed_releases_first_two_weeks %>% 
  count(day, time_category) %>% 
ggplot(aes(x = day, y = n, fill = reorder(time_category, day))) + 
  geom_col() +
  theme_few(base_size = 16) +
  ylab('# of posts') +
  ggtitle("Reed Releases posts by day in Dec 2012") +
  labs(fill = "") +
  scale_x_date(name = "", date_labels = '%a, %b %d', date_breaks = '1 day', expand = c(0,0)) +
...
```{r}
reed_releases_first_two_weeks %>%
  filter(time_category == 'Finals') %>% 
  mutate(
    rank = dense_rank(date)
  ) %>% 
ggplot(aes(x = ymd_hms(date), y = rank)) + 
  geom_line() +
  theme_few(base_size = 16) +
  ylab('# of posts') +
  ggtitle("Reed Releases cumulative posts in finals week") +
  labs(fill = "") +
  scale_x_datetime(name = "", date_labels = '%a, %H:%M', date_breaks = '6 hours', expand = c(0,0)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
