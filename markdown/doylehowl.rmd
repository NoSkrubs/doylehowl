...
# read in raw data
raw_data <- dbReadTable(con, "doyle_howl_data_temp")
...
posts_words <- posts_words %>% left_join(posts_tstamps)
...
page_stats <- data %>% 
  ...
  mutate(
    "Words per post" = round(`Unique words` / Posts, 2)
  )
...
```{r echo=FALSE}
# rereves_words <- page_words %>% filter(page == "Re-Rereves")
# wordcloud(
#     words = rereves_words$word
#   , freq = rereves_words$freq
#   , max.words = 200
#   , fixed.asp = FALSE
#   , random.order=FALSE
#   , rot.per=0
...
trigger_posts <- data %>%
  filter(
    str_detect(str_to_lower(text), "tw:") | str_detect(str_to_lower(text), "cw:")
   ) %>%
  ungroup() %>%
  select(text) %>%
  unnest_tokens(word, text, to_lower = TRUE) %>%
